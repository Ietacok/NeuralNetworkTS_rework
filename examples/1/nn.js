import * as ScalingFunctions from "./ScalingFunctions.js"
...
export class Neuron {
  ...
  Iteration = 0
  SubIteration = 0 //Used only for building of the CalculatedConnectionsPlay
  MaxSubIterations = 1 //Used only for building of the CalculatedConnectionsPlay
...
export class Network {
  ...
  InputNeurons = [] //Rule for updating CurrentIteration within input neurons is a bit loose. (the update might happen depending on how the neural network is built, but this feature nontheless was in intentions and also came in handy with setting values for the input neurons)

  OutputNeurons = []
  OutputNeuronsIsActivated = [] //this feature allows for the AI to perform multiple outputs at the same time

  Connections = [] //[Neuron1Id][num] :  Connection
  ConnectionsBack = [] //[Neuron2Id][num] : Connection

  CalculatedConnectionsPlay = []

  processConnection(Connection) {
    let NeuronA = this.Neurons[Connection.NeuronId1]
    let NeuronB = this.Neurons[Connection.NeuronId2]

    let Val1 = Connection.ScalingFunction(
      (NeuronA.ScalingFunction(NeuronA.Function_value) + NeuronA.Last_nominal) *
    ...
    while (Iteratable.length > 0) {
      for (let i = 0; i < Iteratable.length; i++) {
        ...
        Connections.forEach(Connection => {
          ...
          let Neuron = this.Neurons[Connection.NeuronId2]
  ...
  destroy(NeuronId) {
    ...
    if (Ind > 0) {
      this.OutputNeurons[Ind] = this.OutputNeurons[
        this.OutputNeurons.length - 1
      ]
  ...
  add(Type, CallbackFunc = null, NeuronScalingFunction) {
  ...
  disconnect(Connection) {
    this.Connections[Connection.NeuronId1].slice(
      Connection.NeuronId2,
      Connection.NeuronId2
    )
    this.ConnectionsBack[Connection.NeuronId2].slice(
      Connection.NeuronId1,
      Connection.NeuronId1
    )
  }

  connect(NeuronId1, NeuronId2, Weight, ScalingFunction) {
    let Connections = this.Connections[NeuronId1]
    let Connection_ = new Connection(
      NeuronId1,
      NeuronId2,
      Weight,
      ScalingFunction
  ...
  predictValue() {
    if (this.CalculatedConnectionsPlay.length === 0) {
      console.error(
        "For the function predict_value to work, you must calculate CalculatedConnectionsPlay."
      )
      return false
    }
    ...
    for (let i = 0; i < this.CalculatedConnectionsPlay.length; i++) {
      let Connection = this.CalculatedConnectionsPlay[i]
  ...
  iterate() {
    ...
    let ActiavatedOutputNeurons = []
    let ActivationValues = [] //this adds flexibility to your model

    for (
      let out_neuron_id = 0;
      out_neuron_id < this.OutputNeurons.length;
      out_neuron_id++
    ) {
      let OutputNeuron = this.OutputNeurons[out_neuron_id]
      let ActivationValue = this.OutputNeuronsIsActivated[out_neuron_id](
        OutputNeuron.Function_value
      )
